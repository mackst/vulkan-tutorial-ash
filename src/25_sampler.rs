...
impl HelloTriangleApplication {
    ...
    fn create_graphics_pipeline(&mut self) {
        let vert_shader_module = self.create_shader_module("22_shader_ubo_vert.spv".to_owned());
        let frag_shader_module = self.create_shader_module("22_shader_ubo_frag.spv".to_owned());
    ...
    fn create_texture_image(&mut self) {
        ...
        let texture_path = exe_path.parent().unwrap().join("texture.jpg");
    ...
    fn create_texture_image_view(&mut self) {
        self.texture_image_view =
            Some(self.create_image_view(self.texture_image.unwrap(), vk::Format::R8G8B8A8_SRGB));
    }
    ...
    fn create_image_view(&mut self, image: vk::Image, format: vk::Format) -> vk::ImageView {
    ...
    fn create_vertex_buffer(&mut self) {
        ...
        unsafe {
            ...
            data_slice.copy_from_slice(&VERTICES);
            self.device
                .as_ref()
                .unwrap()
                .unmap_memory(staging_buffer_memory);
    ...
    fn create_index_buffer(&mut self) {
        ...
        unsafe {
            ...
            data_slice.copy_from_slice(&INDICES);
            self.device
                .as_ref()
                .unwrap()
                .unmap_memory(staging_buffer_memory);
